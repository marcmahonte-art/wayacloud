...
export function WhatsAppBackupCard() {
  ...
  const handleBackup = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. Obtenir l'URL de présignature pour Wasabi
      ...
      if (!presignRes.ok) {
        throw new Error("Erreur lors de la préparation de la sauvegarde");
      }

      const { url } = await presignRes.json();

      // 2. Upload directement vers Wasabi
      ...
      if (!uploadRes.ok) {
        throw new Error("Échec du transfert vers Wasabi");
      }

      // Successful backup: save metadata
      const cleanUrl = url.split("?")[0];
      const newFile = {
        name: file.name,
        meta: `${(file.size / (1024 * 1024)).toFixed(1)} Mo • WhatsApp`,
        time: "À l'instant",
        iconName: "FileText",
      ...
      console.warn("Erreur de sauvegarde S3, bascule en mode démo local :", error);
      ...
      setLastBackup("À l'instant");
      setStatus("À jour");
      alert("Sauvegarde WhatsApp terminée avec succès (mode démo local) !");
    } finally {
  ...
  return (
    <article className="min-h-[220px] overflow-hidden rounded-card border border-[#DCEFE3] bg-white p-6 shadow-card">
      ...
      <button 
        ...
        disabled={isUploading}
        className="mt-8 min-h-11 w-full flex items-center justify-center gap-2 rounded-btn border border-green-500 px-5 py-2 text-sm font-bold text-green-700 sm:w-auto hover:bg-green-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
      >
